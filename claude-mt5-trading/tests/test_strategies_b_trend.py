"""Tests des stratégies propres de la famille B (suivi de tendance) : B01 … B10.
...
from tradinglab.agents.strategies import b_trend  # noqa: F401 - l'import enregistre les stratégies
...
AGENT_IDS = [f"B{i:02d}" for i in range(1, 11)]
...
def _check_candidate(c, spec, snap):
    ...
    # distance entrée→SL entre 0,3 et 3 ATR du tf d'entrée (au moins un ATR de référence positif)
...
def test_module_loaded():
    ...
    assert "b_trend" in strategies.LOADED
    assert "b_trend" not in strategies.FAILED
