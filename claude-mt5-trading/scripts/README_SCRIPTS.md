...
| `stop_all.ps1` | Lit `state\pids.json`, envoie `python -m tradinglab.api.cli PAUSE`, attend la prise en compte effective (jusqu'à 20 s, sinon retire la PAUSE non traitée de la file), puis `Stop-Process` (watchdog → orchestrateur → dashboard). **Ne ferme jamais MetaTrader 5.** Options : `-GraceSec`, `-SkipPause`. | Oui (arrêt des processus) |
